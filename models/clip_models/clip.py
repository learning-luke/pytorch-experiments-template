    ...
    if os.path.isfile(download_target):
        if (
            ...
            warnings.warn(
                ...
                f"not match; re-downloading the file"
            )
...
def load(
    ...
    ----------
    name : str
        A model name listed by `clip.available_models()`, or the path to a model
    ...
    jit : bool
        Whether to load the optimized JIT model (default) or more hackable
        non-JIT model.
    ...
    try:
        ...
        if jit:
            warnings.warn(
                f"File {model_path} is not a JIT archive. Loading as a state dict "
                f"instead "
            )
            jit = False
