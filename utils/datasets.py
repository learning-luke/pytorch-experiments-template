        ...
        for augment in aug:
            ...
            if augment == "color_jitter":
                transforms_list.append(
                    transforms.ColorJitter(brightness=0.3, contrast=0.3, hue=0.0)
                )
            ...
            if augment == "affine":
                ...
                transforms_list.append(
                    transforms.RandomAffine(
                        ...
                        else (scale_low, scale_high),
                        shear=shear_degrees,
                    )
                )
            if augment == "random_crop":
                transforms_list.append(
                    transforms.RandomCrop(size=[image_length, image_length], padding=4)
                )

        transform = (
            transforms.Compose(transforms_list)
            if "random_order" not in aug
            else transforms.Compose(
                [transforms.RandomOrder(transforms=transforms_list)]
            )
        )
        ...
        for augment in aug:
            ...
            if augment == "cutout":
                transform.transforms.append(
                    Cutout(n_holes=cut_n_holes, length=cut_length)
                )

        if which_set == "MNIST":
            self.dataset = datasets.MNIST(
                root="../data/MNIST" if root is None else root,
                train=train,
                download=download,
                transform=transform,
            )
            ...
            self.dataset = datasets.FashionMNIST(
                root="../data/Fashion-MNIST" if root is None else root,
                train=train,
                download=download,
                transform=transform,
            )
            ...
            self.dataset = datasets.CIFAR100(
                root="../data/cifar-100" if root is None else root,
                train=train,
                download=download,
                transform=transform,
            )
            ...
            self.dataset = datasets.ImageFolder(
                root=("../data/Cinic-10" if root is None else root)
                + ("/train" if train else "/test"),
                transform=transform,
            )
        else:
            self.dataset = datasets.CIFAR10(
                root="../data/cifar-10" if root is None else root,
                train=train,
                download=download,
                transform=transform,
            )
...
def load_dataset(args):
    ...
    in_shape = (
        (28, 28, 1) if "MNIST" in args.dataset else (32, 32, 3)
    )  # Woohoo, great variety here.

    data_dir = args.root
    root = (
        None
        if args.root.lower() == "default"
        else "{}/{}".format(data_dir, args.dataset)
    )
    ...
    if args.test == 0:
        ...
        for ci in range(num_classes):
            idxs_this_ci = np.random.choice(
                (np.array(labels) == ci).nonzero()[0], num, replace=False
            )
